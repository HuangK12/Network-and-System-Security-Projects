...
static struct nf_hook_ops icmpFilterHook, telnetFilterHook;
...
unsigned int icmpFilter(void *priv, struct sk_buff *skb,
   ...
   if (iph->protocol == IPPROTO_ICMP && iph->daddr == ip_addr) {
     printk(KERN_INFO "Dropping ICMP packet to %d.%d.%d.%d\n",
...
unsigned int telnetFilter(void *priv, struct sk_buff *skb,
  ...
  if (iph->protocol == IPPROTO_TCP && iph->daddr == ip_addr) {
...
int registerFilter(void) {
   ...
   */
   ...
   nf_register_net_hook(&init_net, &icmpFilterHook);
...
void removeFilter(void) {
   ...
   */
   ...
   nf_unregister_net_hook(&init_net, &icmpFilterHook);
